...
import WebRtcStreamer from './rhea-streamer.js';
...
function pushTranscriptToRheaCockpit(text, isFinal) {
    try {
        if (!isFinal) return;
        if (!text || !String(text).trim()) return;

        const root = document.getElementById('rheaCockpitRoot');
        if (root) {
            root.classList.remove('active');
        }

        window.RheaCockpit?.handleTranscript({
            from: ANDROID_XR_ID || 'Provider',
            to: 'AIERIA',
        ...
        console.warn('[RHEA-COCKPIT] Failed to push transcript to same-page cockpit:', err);
    }
}
...
function finalizeRecordingNote() {
    ...
    if (conversationBuffer.trim()) {
        const finalTranscript = conversationBuffer.trim();

        sendTranscript(finalTranscript, true);
        pushTranscriptToRheaCockpit(finalTranscript, true);
