...
async function loadUserFormOptions() {
  try {
    ...
    const { clinics, screens, managers, systems } = data.options;
    ...
    if (Array.isArray(systems)) {
      userFormDynamicOptions.systems = systems.map(s => ({
        id: s.id,
        name: s.app || s.name,
      }));
    } else {
      userFormDynamicOptions.systems = [];
    }
...
const baseCategoryConfig = {
  [USER_CATEGORY.PROVIDER]: {
    ...
    formFields: [
      ...
      { name: 'appId', label: 'App', type: 'select', required: true, optionsKey: 'systems' },
...
function initAssignUsersTopPanel() {
  ...
  if (saveAssignmentBtn) {
    saveAssignmentBtn.onclick = async () => {
      ...
      const selectedProvider = (assignUsersOptions.providers || []).find(
        (p) => String(p.id) === String(providerId)
      );
      const isRheaProvider = !!selectedProvider?.isRheaProvider;
      ...
      try {
        ...
        if (!isRheaProvider) {
          payload.scribeUserId = Number(scribeId);
        }
...
function initAssignUsersDualView() {
  ...
  async function loadAssignProvidersTable() {
    ...
    // Keep the single filtered list; no separate arrays needed here
  ...
  <div class="overflow-x-auto">
    <table class="w-full text-left border-collapse">
      ...
      const isRheaProvider = !!p.isRheaProvider;
      ...
      if (isRheaProvider) {
        return `
              <tr class="border-b border-gray-800"
                  data-unmapped-provider-id="${escapeHtmlInline(pid)}"
                  data-is-rhea-provider="1">
...
`;
    ...
    if (!unmappedTbody.dataset.bound) {
      ...
      unmappedTbody.addEventListener('click', async (e) => {
        ...
        if (!isRheaProvider && (!Number.isFinite(scribeIdNum) || scribeIdNum <= 0)) {
        ...
        try {
          ...
          if (!isRheaProvider) {
            payload.scribeUserId = scribeIdNum;
          }
    ...
    try {
      ...
      providerTableBody.innerHTML = filtered.map((m) => {
        ...
        const isRheaProvider =
          !!m.isRheaProvider || String(p.appName || '').trim().toUpperCase() === 'RHEA';
...
<tr class="table-row border-b border-gray-700"
    data-scribe-id="${escapeHtml(effectiveScribeId)}"
    data-provider-id="${escapeHtml(providerIdForRow)}"
    data-orig-manager="${escapeHtml(s.managerName || s.manager_name || '')}"
    data-orig-scribe="${escapeHtml(effectiveScribeId)}"
    data-is-rhea-provider="${isRheaProvider ? '1' : '0'}">
  ...
  loginForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (response.ok && data.ok) {
        ...
        // Check if user is a Provider - redirect by selected app
        if (data.persona && data.persona.toLowerCase() === 'provider') {
          ...
          if (appName === 'rhea' || appId === '2') {
            window.location.replace('/rhea');
          } else {
    ...
    if (filteredMappings.length) {
      tableBody.innerHTML = filteredMappings
        .map((m) => {
          const s = m.scribe || {};
          const p = m.provider || {};
          const isRheaProvider = !!m.isRheaProvider || String(p.appName || '').trim().toUpperCase() === 'RHEA';
