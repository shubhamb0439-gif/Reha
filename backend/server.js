...
app.get(['/rhea', '/rhea/'], sendView('rhea.html'));
...
app.post('/api/platform/login', async (req, res) => {
  try {
    ...
    let appInfo = { appId: null, appName: null };
    ...
    SELECT TOP 1
      s.id         AS appId,
      s.[system]   AS appName
    FROM [dbo].[User_System_Mapping] usm
    JOIN [dbo].[Systems] s
      ON usm.system_id = s.id
     AND s.row_status = 1
    WHERE usm.user_id = :userId
      AND usm.row_status = 1
    ...
    `,
      ...
      if (appRows && appRows.length > 0) {
        appInfo = {
          appId: appRows[0].appId,
          appName: appRows[0].appName,
        };
      }
    }
...
app.get('/api/platform/me', async (req, res) => {
  try {
    ...
    let appMap = { appId: null, appName: null };

    if (String(u.persona || '').toLowerCase() === 'provider') {
      const appRows = await sequelize.query(
        `
    SELECT TOP 1
      s.id         AS appId,
...
app.post('/api/platform/create-user', requireLogin, requireScreenWrite(6), async (req, res) => {
  ...
  try {
    ...
    if (normalizedCategory === 'provider' && !appId) {
      return res.status(400).json({
        ok: false,
        message: 'App is required for Provider'
      });
    }
  ...
  `,
      ...
      if (normalizedCategory === 'provider' && createdUserId && appId) {
        await sequelize.query(
          `
    INSERT INTO [dbo].[User_System_Mapping] (
      user_id,
      system_id,
      created_date,
      created_by,
...
`;
  ...
  SELECT
    ...
    su.xr_id,
    su.clinic_id,
    usm.system_id AS app_id,
    s.[system]    AS app_name
  FROM [dbo].[System_Users] su
  LEFT JOIN [dbo].[User_System_Mapping] usm
    ON usm.user_id = su.id
   AND usm.row_status = 1
  LEFT JOIN [dbo].[Systems] s
    ON s.id = usm.system_id
   AND s.row_status = 1
  WHERE
    su.row_status = 1
    AND su.status_id = 1
...
`;
    ...
    return res.json({
      ok: true,
      scope: isMasterAdmin ? 'all' : 'manager',
      scribes,
      providers: (providers || []).map((p) => ({
        ...p,
        isRheaProvider: String(p.app_name || '').trim().toUpperCase() === 'RHEA'
      })),
    });
  } catch (err) {
...
app.get('/api/platform/scribe-provider-mapping', requireLogin, requireScreen(8), async (req, res) => {
  ...
  SELECT
    ...
    -- Provider app
    usm.system_id AS provider_app_id,
    sys.[system]  AS provider_app_name,
  ...
  LEFT JOIN [dbo].[User_System_Mapping] usm
    ON usm.user_id = p.id
   AND usm.row_status = 1
  LEFT JOIN [dbo].[Systems] sys
    ON sys.id = usm.system_id
   AND sys.row_status = 1
  WHERE
    m.row_status = 1
    AND (
      :isMasterAdmin = 1
  ...
  SELECT
    ...
    -- Synthetic AI scribe side for RHEA
    NULL         AS scribe_id,
    'AIERIA'     AS scribe_name,
    ''           AS scribe_email,
    ''           AS scribe_xr_id,
    ...
    -- No manual manager for AI row
    NULL         AS scribe_manager_name
  FROM [dbo].[System_Users] p
  JOIN [dbo].[User_System_Mapping] usm
    ON usm.user_id = p.id
   AND usm.row_status = 1
  JOIN [dbo].[Systems] sys
    ON sys.id = usm.system_id
   AND sys.row_status = 1
  LEFT JOIN [dbo].[Clinics] c
    ON p.clinic_id = c.id
  ...
  WHERE
    ...
    AND NOT EXISTS (
      SELECT 1
      FROM [dbo].[Scribe_Provider_Mapping] m2
      WHERE m2.provider_user_id = p.id
        AND m2.row_status = 1
    )
  ...
  `,
    ...
    const mappings = rows.map((r) => {
      const isRheaProvider =
        String(r.provider_app_name || '').trim().toUpperCase() === 'RHEA';

      return {
        id: r.id,
        isRheaProvider,
        scribe: {
          id: isRheaProvider ? null : r.scribe_id,
          name: isRheaProvider ? 'AIERIA' : r.scribe_name,
          email: isRheaProvider ? '' : r.scribe_email,
          xrId: isRheaProvider ? '' : r.scribe_xr_id,
          managerName: isRheaProvider ? null : (r.scribe_manager_name || null),
        },
        provider: {
          id: r.provider_id,
          ...
          appId: r.provider_app_id || null,
          appName: r.provider_app_name || null,
        },
      };
    });
...
app.post('/api/platform/scribe-provider-mapping', requireLogin, requireScreenWrite(8), async (req, res) => {
  ...
  try {
    ...
    let providerAppName = null;

    if (providerId) {
      const providerAppRows = await sequelize.query(
        `
    SELECT TOP 1
      s.[system] AS appName
    FROM [dbo].[User_System_Mapping] usm
    ...
    `,
      ...
      providerAppName = providerAppRows?.[0]?.appName || null;
    }

    const isRheaProvider =
      String(providerAppName || '').trim().toUpperCase() === 'RHEA';

    if (!providerId) {
      return res.status(400).json({
        ok: false,
        message: 'providerUserId is required',
      });
    }

    if (!isRheaProvider && !scribeId) {
      return res.status(400).json({
        ok: false,
    ...
    if (isRheaProvider) {
      return res.json({
        ok: true,
        message: 'RHEA provider uses AIERIA. No manual scribe mapping saved.',
      });
    }
...
`;
    ...
    const [personas, departments, types, statuses, clinics, screens, managers, systems] =
      await Promise.all([
        ...
        sequelize.query(managersQuery, { type: Sequelize.QueryTypes.SELECT }),
        sequelize.query(systemsQuery, { type: Sequelize.QueryTypes.SELECT }),
      ]);
